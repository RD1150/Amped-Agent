...
export default function PublicBuyerPresentation() {
  const params = useParams<{ id: string }>();
  const id = parseInt(params.id ?? "0", 10);

  const { data: pres, isLoading, error } = trpc.buyerPresentation.getPublic.useQuery(
    { id },
  ...
  useEffect(() => {
    ...
    const market = pres.marketCity ? `${pres.marketCity}${pres.marketState ? `, ${pres.marketState}` : ""}` : "";
    const buyerLabel = pres.buyerName ? `for ${pres.buyerName}` : "";
    const title = `Buyer Consultation ${buyerLabel}${market ? ` · ${market}` : ""} — Presented by ${agent}`;
    const description = `Your personalized buyer consultation presentation${market ? ` for the ${market} market` : ""}. Presented by ${agent}. Schedule a call to get started.`;
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-gray-500 text-sm">Loading your buyer consultation…</p>
        </div>
      </div>
    );
  }
  ...
  const marketLabel = [pres.marketCity, pres.marketState].filter(Boolean).join(", ");
            ...
            <div className="min-w-0">
              {pres.buyerName && (
                <div className="text-sm font-semibold text-gray-900 truncate">
                  Buyer Consultation for {pres.buyerName}
                </div>
              )}
              {marketLabel && (
              ...
              {!pres.buyerName && !marketLabel && (
                <div className="text-sm font-semibold text-gray-900">Buyer Consultation</div>
              )}
            </div>
          </div>
      ...
      {pres.gammaUrl ? (
        ...
        <div className="flex items-center justify-center" style={{ height: "calc(100vh - 61px)" }}>
          <div className="text-center space-y-4 max-w-md px-6">
            ...
            {pres.bookingUrl && (
              <Button asChild className="bg-blue-600 hover:bg-blue-500 text-white font-semibold gap-2">
                <a href={pres.bookingUrl} target="_blank" rel="noopener noreferrer">
                  <Calendar className="h-4 w-4" />
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-sm border-t border-gray-100 px-4 py-2">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <p className="text-xs text-gray-400">
            Powered by <span className="font-semibold text-blue-600">Amped Agent</span>
          </p>
          {pres.bookingUrl && (
            <a
