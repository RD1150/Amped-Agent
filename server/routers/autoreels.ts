...
Write a caption that expands on the video content and includes a strong CTA. NO HASHTAGS.`
  ...
  /**
   * Dismiss a stuck reel — marks it as failed so it no longer shows as "rendering".
   * Used when a reel has been processing for too long (render never started or webhook missed).
   */
  dismissStuckReel: protectedProcedure
...
// ============================================================
// SERVER STARTUP RECOVERY FOR AI REELS
// Called once on boot to resolve any reels that were in-flight
// when the server last shut down. Reels with no renderId will
// never complete — mark them failed so agents can retry.
// Reels with a renderId are polled once against Creatomate to
// capture any completed renders that missed their webhook.
// ============================================================
export async function recoverStuckAiReels(): Promise<void> {
  try {
    const { getDb } = await import("../db");
    const { aiReels } = await import("../../drizzle/schema");
    const { eq, and, isNull, isNotNull } = await import("drizzle-orm");
    const { checkRenderStatus } = await import("../_core/creatomateRenderer");

    const database = await getDb();
    if (!database) return;

    // 1. Reels stuck with NO render ID — render never started, mark failed immediately
    const noRenderIdReels = await database
      .select({ id: aiReels.id })
      .from(aiReels)
      .where(and(eq(aiReels.status, "processing"), isNull(aiReels.shotstackRenderId)));

    if (noRenderIdReels.length > 0) {
      const ids = noRenderIdReels.map((r) => r.id);
      console.log(`[Startup Recovery] Found ${ids.length} reel(s) with no render ID — marking as failed: ${ids.join(", ")}`);
      for (const { id } of noRenderIdReels) {
        await database
          .update(aiReels)
          .set({ status: "failed" })
          .where(eq(aiReels.id, id));
      }
    } else {
      console.log("[Startup Recovery] No stuck AI reels without render ID found.");
    }

    // 2. Reels stuck WITH a render ID — poll Creatomate once to see if they completed
    const pendingReels = await database
      .select({ id: aiReels.id, shotstackRenderId: aiReels.shotstackRenderId })
      .from(aiReels)
      .where(and(eq(aiReels.status, "processing"), isNotNull(aiReels.shotstackRenderId)));

    if (pendingReels.length > 0) {
      console.log(`[Startup Recovery] Polling ${pendingReels.length} reel(s) with render IDs…`);
      for (const reel of pendingReels) {
        try {
          const result = await checkRenderStatus(reel.shotstackRenderId!);
          if (result.status === "done" && result.url) {
            await database
              .update(aiReels)
              .set({ status: "completed", shotstackRenderUrl: result.url })
              .where(eq(aiReels.id, reel.id));
            console.log(`[Startup Recovery] Reel ${reel.id} completed — URL saved.`);
          } else if (result.status === "failed") {
            await database
              .update(aiReels)
              .set({ status: "failed" })
              .where(eq(aiReels.id, reel.id));
            console.log(`[Startup Recovery] Reel ${reel.id} failed on Creatomate — marked failed.`);
          } else {
            // Still rendering — leave as-is; user can dismiss from My Content
            console.log(`[Startup Recovery] Reel ${reel.id} still rendering (${result.status}) — leaving as processing.`);
          }
        } catch (pollErr) {
          console.error(`[Startup Recovery] Could not poll reel ${reel.id}:`, pollErr);
        }
      }
    } else {
      console.log("[Startup Recovery] No stuck AI reels with render IDs found.");
    }
  } catch (err) {
    // Recovery is best-effort — never crash the server on startup
    console.error("[Startup Recovery] AI reels recovery error:", err);
  }
}
